...
async function getObras() {
  const { data, error } = await supabase
    ...
    .select(
      ...
      "ciudad, direccion, fecha_inicio_plan, fecha_fin_plan, tipo_obra, notas"
    )
    .order("nombre");
  if (error) console.error("[obras/page] getObras error:", error.message);
  // Mapear notas → descripcion para compatibilidad con los componentes hijos
  return (data ?? []).map((o: any) => ({ ...o, descripcion: o.notas }));
}
...
// Valores exactos del enum estado_obra_enum en Supabase
...
export default async function ObrasPage({ searchParams }: Props) {
  ...
  const finalizadas    = todasObras.filter((o: any) => o.estado === "terminada").length;
